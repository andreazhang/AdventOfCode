...
class CubeConundrumTest {
    ...
    fun `parse multiple games`() {
        val input =
            this::class.java.getResource("/adventOfCode2023/day2/basic.txt")?.readText() ?: fail()

        val games = parseGames(input)

        assertAll(
            "games has correct properties",
    ...
    fun `calculate sum of games with restricted number of available cubes full input`() {
        val input =
            this::class.java.getResource("/adventOfCode2023/day2/full.txt")?.readText() ?: fail()
        val games = parseGames(input)

        val total = calculateTotal(games)

        assertEquals(2_268, total)
    }
